...
class ChannelAttentionModule(nn.Module):
    ...
    # 最大池化和平均池化，输出维度 [b, c, 1, 1]
        max_pool = self.max_pool(x)  # 不要用 view(b, c)
        avg_pool = self.avg_pool(x)  # 不要用 view(b, c)

    # 第一个全连接层降通道数 [b, c, 1, 1] => [b, c//4, 1, 1]
    ...
    # 第二个全连接层恢复通道数 [b, c//4, 1, 1] => [b, c, 1, 1]
        x_maxpool = self.fc2(x_maxpool)
        x_avgpool = self.fc2(x_avgpool)

    # 将最大池化和平均池化的结果相加 [b, c, 1, 1]
    ...
    # 输入特征图和通道权重相乘 [b, c, h, w]
        outputs = x * x  # 这里应该是 x * 原始输入 x，而不是 x * x
